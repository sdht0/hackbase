...
echo "Welcome to Hackbase! The Game is ready to be played. Have fun \m/"
echo "Hackbase url: http://$(ip -o -f inet addr | grep -v "127.0.0" | cut -d/ -f1 | sed -r "s/[ \t]+/ /g" | cut -d" " -f4 | head -n1)"
